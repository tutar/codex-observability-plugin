...
- **Checking dedup sidecars** — after telemetry flush succeeds, the Stop hook records uploaded turn ids next to the rollout as `<rollout>.jsonl.langfuse`. The hook payload's exact `turn_id` is treated as stopped even when Codex has not yet persisted `task_complete`; unrelated incomplete turns remain skipped. A failed export stays absent from the sidecar so a later hook or manual replay can retry it.
